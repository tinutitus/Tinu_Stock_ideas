# app.py
# Full integrated NSE Screener — ML optional, Geo-news, logging, auto-actuals, robust fetch & fallback.
...
st.title("⚡ NSE Screener — Full App (ML, Geo-News, Logging)")
...
# ---------------- Constituents (midcap fallback lists) ----------------
INDEX_URLS = {
...
SMALLCAP_FALLBACK = ["AARTIIND","AFFLE","AMBER","ANANDRATHI","APLAPOLLO","ARVINDFASN","ASTERDM","ASTRAZEN","BASF","BAJAJHLDNG"]
...
# ---------------- Batch price fetch (robust) ----------------
@st.cache_data(show_spinner=False)
def batch_history(tickers, years=4):
    # return yfinance result; caller will handle index shapes
...
def compute_hidden_features(s: pd.Series):
    ...
    # lower threshold so more tickers included (was 60)
    if s.size < 30:
        return None
    cur=float(s.iloc[-1])
    def mom(days):
...
def heuristic_ret1m_from_feats(feats):
    base_1m = 0.6 * (feats.get("m21",0.0) or 0.0) + 0.4 * (feats.get("m63",0.0) or 0.0)
    adj_factor = 1.0
...
# safe slider defaults to process all tickers by default
n_companies = len(companies)
min_tickers = 1 if n_companies < 10 else 10
max_tickers = max(1, n_companies)
default_tickers = max_tickers  # default to all
...
try:
    ...
    st.caption("Shows VADER sentiment (avg) and headline volume for 3d / 7d / 30d per region; topic fractions (7d); aggregated geo_news_risk.")
    regions = ["us","eu","cn"]
    ...
    st.markdown("**Topic breakdown (7d) — fraction of headlines containing topic**")
    topic_rows=[]
    for region in regions:
        row = {"region": region.upper()}
        for t in TOPIC_KEYWORDS.keys():
            row[t] = round(geo_snapshot.get(f"news_{region}_topic_{t}_7d", 0.0), 3)
...
# ---------------- Load fundamentals if URL provided ----------------
fund_df = None
if fund_csv_url:
...
st.info(f"Fetching price history for {len(tickers_subset)} tickers — this may take a minute.")
price_data = batch_history(tickers_subset, years=4)
price_map = {}
for t in tickers_subset:
    try:
        # yfinance returns MultiIndex columns when multiple tickers; single-ticker returns flat df
        if isinstance(price_data.columns, pd.MultiIndex):
            ser = price_data[t]["Close"].dropna()
        else:
            # single-ticker response or dataframe: ensure it contains Close
            if "Close" in price_data.columns:
                ser = price_data["Close"].dropna()
            else:
                # sometimes yfinance returns a dict-like for one ticker; fallback to per-ticker fetch
...
# ---------------- Build predictions (with fallback heuristic) ----------------
rows=[]; log_rows=[]
...
for tkr, ser in price_map.items():
    if ser is None or ser.empty:
        skipped_debug.append((tkr, "no_data"))
        continue
    feats = compute_hidden_features(ser)
    # fallback if not enough history: build simple short heuristic using last 6 points
    if feats is None:
        if len(ser) >= 6:
            ...
            skipped_debug.append((tkr, f"insufficient_history({len(ser)})"))
            continue
...
# optional debug output of skipped tickers
if skipped_debug:
    st.info(f"Skipped {len(skipped_debug)} tickers due to missing data or insufficient history (shown below).")
    st.dataframe(pd.DataFrame(skipped_debug, columns=["ticker","reason"]).head(200))

if not rows:
    st.error("No tickers processed (check yfinance data).")
    st.stop()
...
# ---------------- Append to log and compute run_date ranks ----------------
ensure_log_exists()
...
# ---------------- Portfolio builder (default ₹10,000) ----------------
st.markdown("---")
...
st.caption("Notes: predictions are logged in predictions_log.csv. To persist across environments consider Google Sheets/S3/DB integration. ML training can be heavy — enable only when needed.")
