# app.py
# Complete NSE Screener app — ML optional, Geo-News, logging, robust fetch & fallback.
...
MIN_HISTORY_DAYS_FOR_FEATURES = 30  # lower threshold so more tickers included; tune as needed
...
# ---------------- Constituents (Nifty Midcap 100 + fallback) ----------------
INDEX_URLS = {
...
# safe slider defaults (avoid StreamlitAPIException)
...
try:
    st.markdown("### 🔎 Geo-political News Snapshot (rolling)")
    st.caption("Shows VADER sentiment (avg) and headline volume for 3d / 7d / 30d per region; topic fractions (7d); geo_news_risk.")
    regions = ["us","eu","cn"]
    ...
    st.warning("Geo-news snapshot failed to render (check network/RSS).")
...
# ---------------- Fetch price history & build price_map robustly ----------------
...
# ---------------- Build predictions (ML if available; fallback heuristic) ----------------
rows=[]; log_rows=[]
...
# optional debug of skipped tickers
if skipped_debug:
...
st.caption("Notes: predictions use geo-news + fundamentals (if provided). Logs saved to predictions_log.csv. Consider Google Sheets/S3 for shared persistence. ML training can be heavy — enable only when you want to train.")
