# app.py
# Complete integrated NSE Screener: ML (1M & 1Y) + Geo-News + Fundamentals + Logging + Actuals + UI
...
st.set_page_config(page_title="NSE Screener — ML + Geo-News + Logging", layout="wide")
st.title("⚡ NSE Screener — ML (1M & 1Y) + Geo-News + Fundamentals + Logging")
...
MIDCAP_FALLBACK = ["ABBOTINDIA","ALKEM","ASHOKLEY","AUBANK","AUROPHARMA","BALKRISIND","BEL","BERGEPAINT","BHEL","CANFINHOME"]
SMALLCAP250_FALLBACK = ["AARTIIND","AFFLE","AMBER","ANANDRATHI","APLAPOLLO","ARVINDFASN","ASTERDM","ASTRAZEN"]
...
# ---------------- ML dataset builder (Phase-1: news + fundamentals)
@st.cache_data(ttl=86400)
...
# ---------------- show geo-news snapshot prominently
vix = fetch_vix()
...
# GEO-NEWS VISIBILITY CARD
try:
    st.markdown("### 🔎 Geo-political News Snapshot (rolling windows)")
    st.caption("VADER sentiment avg and headline volume for 3d / 7d / 30d; topic fractions (7d); geo_news_risk.")
    regions = ["us","eu","cn"]
    topic_keys = list(TOPIC_KEYWORDS.keys())
    ...
    df_topics = pd.DataFrame(topic_rows).set_index("region")
    st.table(df_topics)
except Exception:
    st.warning("Geo-news snapshot failed to render (check network / feedparser).")

# ---------------- load fundamentals CSV if provided
fund_df = None
if fund_csv_url:
...
st.info(f"Processing {len(tickers_subset)} tickers — this can take a while.")
...
if enable_ml and price_map:
    ...
    st.info("Building ML dataset (this can be slow)...")
...
# ---------------- Build predictions (ML if available; else heuristic)
rows=[]; log_rows=[]
...
if not rows:
    st.error("No rows to display (insufficient data)."); st.stop()
...
# ---------------- Update actuals (automatic / forced)
...
# ---------------- Integrated historical table + summary
...
st.caption("Notes: Models use geo-news + fundamentals (if provided). Predictions are logged and actuals auto-filled when horizons mature. For persistent/shared logs consider Google Sheets/S3/DB integration.")
